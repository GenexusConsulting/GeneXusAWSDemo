...
Para alojar la infraestructura es necesario crear un VPC ( Virtual Private Cloud), la cual conforma la base de todo los demás recursos que pueden ser implementados en nuestra infraestructura. La VPC puede contener tanto redes públicas como privadas según se dispongan o no de “Internet Gateways”. 
...
Para acceder a la instancia RDS de la base de datos creada, utilizaremos las credenciales del usuario que se haya definido en el cdk y como password se tendrá que obtener el “Secreto” creado por nuestro Stack en AWS. La URL de acceso podrá ser obtenida desde el servicios RDS (“Punto de enlace y puerto”)
...
Esto se debe hacer a través de la consola de AWS y debe actualizarse en caso que el token sea invalidado.
...
# se marca como la ultima (es la que usamos para desplegar)
docker tag image-name:latest AWS-ID.dkr.ecr.us-east-1.amazonaws.com/project-name:latest
...
3. Creación de imágen Docker conteniendo nuestra aplicación GX:
    ...
    - en el cdk se pueden hacer uso del “contexto” por medio del archivo cdk.json y definir allí, valores como por ejemplo: ```"DATABASE_PREFIX_ENVIRONMENT": "COM_GXDEMO"```
        ...
        El contexto se puede verificar “cdk context”.

4. Cargar imágen Docker al repositorio ECR:
    ...
    - Cada repositorio mantiene versiones de una única imágen (distintos TAGs).
